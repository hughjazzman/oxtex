...
% for any objects $A,B$ in $\mathcal{P}$,
% the mapping $Y_{A,B}: \mathcal{P}(A,B) \rightarrow [\mathcal{P},\textbf{Set}](Y\ A, Y\ B) $ is bijective.
% want to show that for any objects $A,B$ in $\mathcal{P}$,
% mapping $Y'_{A,B}$ is bijective.
% i.e. show that 

% The only arrows in $[\mathcal{P}^{op},\mathcal{B}]$
...
% Let $\phi: [\mathcal{P}^{op},\textbf{Set}] \rightarrow [\mathcal{P}^{op},\mathcal{B}]$
% be the bijection where
% \[\phi :: \mathcal{P}(-, A) \mapsto 
