...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{Categories, Proofs, and Processes: \\Lecture 18 Monoidal Categories}
%
...
\begin{itemize}
    ...
    \item confluent:
    \[ \forall t_1,t_2. \left(t
    \begin{cases}
        \rightarrow\rightarrow\rightarrow t_1\\
        \rightarrow\rightarrow\rightarrow t_2
    \end{cases}\right),
    \exists t_3. 
    \left(\left.
    \begin{cases}
        t_1 \rightarrow\rightarrow\rightarrow \\
        t_2 \rightarrow\rightarrow\rightarrow 
    \end{cases}
    \right\} t_3\right) \]
\end{itemize}
...
\begin{definition}
    ...
    \[f: A\rightarrow B\]
    \[f: A\otimes B \rightarrow C\otimes D \otimes E\]
    \[f\otimes g\]
    \[\sigma_{A,B} = \texttt{swap}_{A,B}\]
    Symmetry natural isomorphisms.
\end{definition}
\begin{definition}
A symmetric monoidal category (SMC) is a monoidal category with a natural isomorphism
\[\sigma_{A,B} A\otimes B \xrightarrow[]{\sim} B\otimes A\]
satisfying:
\[\sigma_{A,B}^{-1} = \sigma_{B,A}\]
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& {A\otimes (B \otimes C)} & {(B\otimes C)\otimes A} \\
	{(A\otimes B) \otimes C} &&& {B\otimes (C\otimes A)} \\
	& {(B\otimes A)\otimes C} & {B\otimes (A\otimes C)}
	\arrow["\alpha", from=2-1, to=1-2]
	\arrow["{\sigma_{A,B\otimes C}}", from=1-2, to=1-3]
	\arrow["\alpha", from=1-3, to=2-4]
	\arrow["{\sigma_{A,B}\otimes 1_C}"', from=2-1, to=3-2]
	\arrow["\alpha"', from=3-2, to=3-3]
	\arrow["{\sigma_{A,C}}"', from=3-3, to=2-4]
\end{tikzcd}\]
\end{definition}
\begin{note}
Braided monoidal category makes distinction of over or under swap.
\end{note}
\begin{definition}
    A strict SMC is an SMC where $\alpha=1,\lambda=\rho$.
    $(\sigma\neq 1!)$.
\end{definition}

\subsection{Algebriac structures in monoidal categories}
\begin{definition}
    A monoid in a strict monoidal category $(\mathcal{C},\otimes, I)$ is an object $X$
    with morphisms 
    \[\mu : X\otimes X \rightarrow X, \quad \eta: I\rightarrow A\]
    satisfying:
    \begin{itemize}
        \item \underline{associativity}
    % https://q.uiver.app/#q=WzAsNCxbMCwwLCJYXFxvdGltZXMgWFxcb3RpbWVzIFgiXSxbMSwwLCJYXFxvdGltZXMgWCJdLFsxLDEsIlxcYnVsbGV0Il0sWzAsMSwiWFxcb3RpbWVzIFgiXSxbMCwxLCJYXFxvdGltZXMgXFxtdSJdLFsxLDIsIlxcbXUiXSxbMCwzLCJcXG11IFxcb3RpbWVzIFgiLDJdLFszLDIsIlxcbXUiLDJdXQ==
    \[\begin{tikzcd}
        {X\otimes X\otimes X} & {X\otimes X} \\
        {X\otimes X} & X
        \arrow["{X\otimes \mu}", from=1-1, to=1-2]
        \arrow["\mu", from=1-2, to=2-2]
        \arrow["{\mu \otimes X}"', from=1-1, to=2-1]
        \arrow["\mu"', from=2-1, to=2-2]
    \end{tikzcd}\]
        \item \underline{left unit}
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJYIl0sWzEsMCwiWFxcb3RpbWVzIFgiXSxbMSwxLCJYIl0sWzAsMSwiXFxldGEgXFxvdGltZXMgWCJdLFsxLDIsIlxcbXUiXSxbMCwyLCIxX1giLDJdXQ==
    \[\begin{tikzcd}
        X & {X\otimes X} \\
        & X
        \arrow["{\eta \otimes X}", from=1-1, to=1-2]
        \arrow["\mu", from=1-2, to=2-2]
        \arrow["{1_X}"', from=1-1, to=2-2]
    \end{tikzcd}\]
        \item \underline{right unit}
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJYIl0sWzEsMCwiWFxcb3RpbWVzIFgiXSxbMSwxLCJYIl0sWzAsMSwiWFxcb3RpbWVzIFxcZXRhIl0sWzEsMiwiXFxtdSJdLFswLDIsIjFfWCIsMl1d
    \[\begin{tikzcd}
        X & {X\otimes X} \\
        & X
        \arrow["{X\otimes \eta}", from=1-1, to=1-2]
        \arrow["\mu", from=1-2, to=2-2]
        \arrow["{1_X}"', from=1-1, to=2-2]
    \end{tikzcd}\]
    \end{itemize}
\end{definition}
\begin{note}
    \[(-\otimes X) : \mathcal{C} \rightarrow \mathcal{C}\]
    \[f \otimes X := f \otimes 1_X : A\otimes X \rightarrow B\otimes X\]
\end{note}
...
\begin{example}
    ...
    \[1 := \eta(*)\]
\begin{itemize}
    \item Assocativity says
    \[\forall a,b,c\in X, \mu(A,\mu(b,c)) = \mu(\mu(a,b),c)\]
    i.e. $a\cdot (b\cdot c) = (a\cdot b) \cdot c$
    \item Left unit
    \[\forall a\in X, \mu (\eta(*), a) = a\]
    i.e. $(1\cdot a) = a$
    \item Right unit
    \[\forall a\in X, a\cdot 1 = a\]
\end{itemize}
\end{example}

\begin{example}
Monoids in $(\textbf{Ab}, \times, \{0\})$ -
where \textbf{Ab} is the category of abelian groups and group homomorphisms - are rings.
\[(X, \mu: X\times X \rightarrow X, \eta: \{0\} \rightarrow X)\]
as before, but $X$ is an abelian group and $\mu$ and $\eta$ must be group homomorphisms.
...
\begin{example}
Monoids in $(\textbf{Vect}_k, \otimes, k)$ are associative algebras,
$:=$ vector space with multiplication $(-\cdot -)$ which is associative, unital and linear in each argument.
\[\left(\sum_i \lambda_i v_i\right) \cdot w = \sum_i \lambda_i (v_i\cdot w)\]
\[v\cdot \left(\sum_i \lambda_i w_i\right) = \sum_i \lambda_i (v\cdot w_i)\]
\end{example}
