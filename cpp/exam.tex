...
Here, a few definitions which will be used in this paper are listed.
\begin{definition}[Category \cite{Abramsky_2010, nlab:category, Grothendieck:61}]
\label{def:cat}
...
\begin{definition}[\cite{maclane:71, nlab:bicategory, BénabouJean2006Itb}]
...
\end{itemize}
such that the following \textit{coherence} conditions hold
\begin{itemize}
...
\begin{definition}[\cite{maclane:71, nlab:bicategory}]
...
First, we must list the definition of a monoid, with categories being defined as in Def. \ref{def:cat}.
\begin{definition}[Monoid \cite{Abramsky_2010}]
  \label{def:monoid}
    A \textbf{\underline{monoid}} is a structure $(M,\cdot, 1_M)$ where $M$ is a set,
    \[- \cdot - : M\times M \rightarrow M\]
    is a binary operation, and $1_M \in M$, satisfying the axioms $\forall x,y,z\in M$.
    \[(x\cdot y)\cdot z = x\cdot (y\cdot z),\quad 1_M\cdot x = x = x\cdot 1_M\]
\end{definition}
...
\end{definition}
\begin{note}
Bicategories are then the generalisation of strict 2-categories - 
...
\begin{definition}[Monad \cite{Abramsky_2010, Sobociński:2017}]
\label{def:monad}
...
\begin{definition}[Monad \cite{STREET1972149, Sobociński:2017}]
\label{def:monad-gen}
Let $\mathcal{C}$ denote any bicategory.
A \textbf{\underline{monad}} $(X,S)$ in $\mathcal{C}$ consists of
\begin{itemize}
  \item A 0-cell $X$
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item The 1-cell is the endofunctor $T: \mathcal{C} \rightarrow \mathcal{C}$
    \item The 2-cells are the natural transformations $\mu: T^2 \rightarrow T, \eta: 1_T \rightarrow T$.
  \end{itemize}
\end{definition}
The notion of monads then make sense for any 2-category.
\begin{definition}[Span \cite{nlab:span, Sobociński:2017}]
...
so given spans $A \xleftarrow[]{f} D \xrightarrow[]{g} B, B\xleftarrow[]{h} E \xrightarrow[]{k}C$,
we can take a pullback of $g$ along $h$ and obtain a span from $A$ to $C$.
...
\end{tikzcd}\]
Maps between spans are then functions $f : C \rightarrow C'$ which make the following diagram commute.
...
\label{ex:mon-spanset}
  Using the bicategory Span(\textbf{Set}) in Def. \ref{def:monad-gen} for a monad, we get
\begin{itemize}
...
\begin{itemize}
  ...
  as defined using $\mu$ and $\eta$ in Ex. \ref{ex:mon-spanset}.
\end{itemize}
To be specific, the type of category referred to so far is a small category, defined as follows.
...
At the same time, the concept of a "monoid-as-a-set" $(M,\cdot,1_M)$ can be generalised to categories.

First, we define the \textit{strict} monoidal category.
\begin{definition}[\cite{Kissinger19:2024}]
...
Monoids in $(\textbf{Set}, \times, \{*\})$ are monoids as in Def. \ref{def:monoid}
...
The (strict) 2-category made of the object $c$ with its category $D(c,c)$ is a full sub-2-category of $D$.
...
\end{proposition}
\begin{proof}
The strict monoidal category consists of
\begin{itemize}
  \item $D(c,c)$ as the category, where objects are 1-cells, and arrows are 2-cells.
  \item The monoidal product $\otimes$ on 1-cells - the composition of 1-cells.
  \item The unit object - the identity cell at $c, 1_c : c\to c$.
  \item 1-cells satisfying the associativity and unit axioms, by definition of the strict bicategory $D$,
  ...
  \item 2-cells satisfying the associativity and unit axioms, by definition of arrows in the category $D(c,c)$
\end{itemize}
...
By using the strict 2-category of (locally) small categories \textbf{Cat}, 
the object $C$ is a category, so $[C,C]$ is the category of endofunctors on $C$.
The following definition can then be obtained.
\begin{definition}[\cite{Kissinger20:2024}]
  A monad is a monoid in the category of endofunctors.
\end{definition}

\subsection{Conclusion}
As expressed in the comment section \cite{Sobociński:2017}, 
there are inconsistencies in using the generalised versus the ordinary definitions of the various terms
- monoids, category, and monads.

In summary, the above show that
\begin{itemize}
  \item A "monoid-as-a-set" is a special type of small category
  \item A small category is a special type of (generalised) monad
  \item A (generalised) monad is a special type of (generalised) monoid
\end{itemize}
Ultimately, there is no proper circular logic, since the start and end are not the same notion of a monoid.
However, the same comment does conjecture on whether it might be possible to do so using an $\infty$-category.
