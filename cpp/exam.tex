...
If $\mathcal{C}$ has pullbacks, we can compose spans, so given a span from $A$ to $B$ and $B$ to $C$,
...
\end{itemize}
\end{example}
Here the monad of Span(\textbf{Set}) can be described as a category where
\begin{itemize}
...
\end{itemize}
Then, we have that this specific category as being a special case 
of a monad on the specific bicategory of Span(\textbf{Set}).
...
Here we give definitions to aid in the understanding of a monoid in \textbf{Set}. 
\begin{definition}[\cite{Kissinger:2024}]
...
\begin{example}[\cite{Kissinger:2024}], where
\begin{itemize}
