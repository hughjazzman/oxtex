...
Observe that for $I,J\in\mathcal{I}(G),I\neq J$,
\begin{itemize}
...
\begin{equation}
\begin{aligned}
\pi_I P_{I,J} &= \frac{\lambda^{|I|}}{Z(G)}\frac{\lambda}{2n(1+\lambda)}\\
&=\frac{\lambda^{|I|+1}}{Z(G)}\frac{1}{2n(1+\lambda)}\\
&=\frac{\lambda^{|J|}}{Z(G)}\frac{1}{2n(1+\lambda)}\\
&=\pi_J P_{J,I}
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation}
where $n=|V|$. Note that since $0<\lambda<\frac{1}{2}, -1<\frac{2\lambda - 1}{1+\lambda}<0$.
...
\[\tau(\varepsilon)=O\left(\frac{2n\ln(\frac{n}{\varepsilon})}{1-2\lambda}\right)\]
...
that it reaches the sheep going clockwise, or anticlockwise.
...
% \begin{enumerate}
%     \item The fox starts at $0$,  $i-1$, then goes back around to $i+1$, before
% \end{enumerate}
This is just the gambler's fortune from the lecture notes, with $a=n-1,b=1-n$.

Then $Z_0,Z_1,...,$ is a martingale as 
\[\mathbb{E}[Z_{n+1}|Z_0,...,Z_n]=\frac{1}{2} (Z_n + 1 + Z_n - 1) = Z_n\]
Let $T$ be the stopping time for $Z_1,Z_2,...,$.
The second condition of the optional stopping theorem holds as 
\[\forall i, |Z_{\min\{i,T\}}|\leq n\]
Thus, $\mathbb{E}[Z_T]=\mathbb{E}[Z_0]=0$.

For $i=0,...,n-1$, denote
\[s_i=\min\{t\geq 1:X_t=i\}\]
\[r_i=\min\{t\geq 1:X_t=i \wedge M_{t+1}-M_t=1\}\]
\[r_{n-i}=\min\{t\geq 1:X_t=i \wedge M_{t+1}-M_t=-1\}\]
so $r_i,r_{n-i}$ is the first time that $X_t=i$ with the previous step being 
clockwise and anticlockwise respectively.
We have $s_i=\min\{r_i,r_{n-i}\}$.
Since $Z_t$ is a martingale, $\forall i,\mathbb{E}[Z_{s_i}]=0$.
We want to find the $\argmax_{i} \Pr(s_i=T)$.

For $i=1$,
\[\Pr(s_1=T)=\Pr(s_2<s_1)=\Pr(r_1>r_{n-1})\]
Then the moment the fox eats the sheep at $i=1$
\begin{equation}
\begin{aligned}
\mathbb{E}[Z_{s_1}]&=\Pr(r_1 < r_{n-1}) + (1-n)\Pr(r_{n-1} < r_1)\\
&=1-\Pr(s_1=T) + (1-n)\Pr(s_1=T)\\
&=0
\end{aligned}
\end{equation}
Thus,
...
\begin{equation}
\begin{aligned}
\mathbb{E}[Z_{s_i}]&=i\Pr(r_i < r_{n-i}) + (i-n)\Pr(r_{n-i} < r_i)\\
&=i\Pr(r_i < r_{n-i}) + (i-n)(1-\Pr(r_i < r_{n-i}))\\
&=0
\end{aligned}
\end{equation}
Thus,
\[\Pr(r_i < r_{n-i})=\frac{n-i}{n}\]
\[\Pr(r_i > r_{n-i})=\frac{i}{n}\]

\begin{equation}
\begin{aligned}
\Pr(s_i=T)&=\Pr(s_i=T|s_{i-1} < s_{i+1})\Pr(s_{i-1} < s_{i+1}) + \Pr(s_i=T|s_{i+1} < s_{i-1})\Pr(s_{i+1}< s_{i-1})
\end{aligned}
\end{equation}
