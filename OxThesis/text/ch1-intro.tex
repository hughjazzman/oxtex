...
since classical simulation is a fundamental aspect of understanding and designing quantum hardware, 
...
First, from this chapter, we introduce the basic notation and concepts of quantum computing,
with a focus on the ZX-Calculus.
Next, \autoref{ch:2-litreview} outlines the methods used for classical simulation via stabiliser decomposition
using ZX-Calculus as covered in \cite{Kissinger_2022, Kissinger2022cat,Codsi2022}.
\autoref{ch:3-methods} formalises the definition of using heuristics to improve classical simulation,
and connects these heuristics to the existing methods in the literature 
\cite{Codsi2022,sutcliffe2024proccut,codsi2023}.
The classes of circuits benchmarked are introduced in \autoref{ch:4-results},
as well as the numerical results of using heuristics covered in \autoref{ch:3-methods}.
It also discusses the results and some of the insights gained from the experiments.
Finally, \autoref{ch:5-conclusion} concludes the thesis with a summary of the results and future work.

The bulk of novel work in this thesis is in \autoref{ch:3-methods} and \autoref{ch:4-results},
where the heuristics are introduced and the results are presented.

\section{Basic Notation}
\label{sec:basic-notation}
\textit{
  This section, and the one after it, introduce the basic notation and concepts of quantum computing,
  along with the ZX-Calculus, and is a subset of the concepts covered by Coecke and Kissinger  
  in Picturing Quantum Processes \cite{coecke2017picturing}.
}
...
For $\alpha,\beta\in[0,2\pi)$ and $a\in\{0,1\}$, the rules are as follows in \autoref{fig:zx-rules}.
...
The names of the rules in \autoref{fig:zx-rules} are spider $\SpiderRule$usion, colour change \CCRule rule,
...
\textit{
  This section connects the ZX-diagrams to graphs, in a way which will continue to be used
  for the rest of the thesis.
  The content in this section will be covered by Kissinger and van de Wetering 
  in the soon-to-be-published Picturing Quantum Software \cite{Kissinger2024}.
  Most of the concepts were also covered in \cite{Duncan2020graphtheoretic,Kissinger2020}
}
